...
def distribute(depth, _x, _y, size_x, size_y, image_height, image_width):
    ...
    extended_x = _x.unsqueeze(2).repeat([1, 1, size_x]) + _i  # [B, N, size_x]
    extended_y = _y.unsqueeze(2).repeat([1, 1, size_y]) + _j  # [B, N, size_y]

    extended_x = extended_x.unsqueeze(3).repeat([1, 1, 1, size_y])  # [B, N, size_x, size_y]
    extended_y = extended_y.unsqueeze(2).repeat([1, 1, size_x, 1])  # [B, N, size_x, size_y]

    extended_x.ceil_()
    extended_y.ceil_()

    value = depth.unsqueeze(2).unsqueeze(3).repeat([1, 1, size_x, size_y])  # [B, N, size_x, size_y]
    ...
    weight = (masked_points.float()
          * (1 / (value + epsilon)))  # [B, N, size_x, size_y]
...
def points2depth(points, image_height, image_width, size_x=4, size_y=4,scale=1):
    ...
    coord_x = (points[:, :, 0] / (points[:, :, 2] + epsilon)) * (image_width / image_height)*scale  # [B, N]
    coord_y = (points[:, :, 1] / (points[:, :, 2] + epsilon)) * scale  # [B, N]
...
# 方法 1：使用 plyfile 保存 PLY 文件（推荐）
def save_to_ply(points, filename='point_cloud.ply'):
    from plyfile import PlyData, PlyElement

    """
    将点云数据保存为 PLY 文件
    参数:
        points: 形状为 (N, 3) 的 NumPy 数组，包含 (x, y, z) 坐标
        filename: 输出 PLY 文件名
    """
    # 确保数据类型为 float32
...
if __name__=='__main__':
    ...
    points = np.load('/mnt/sda/xxy/Project/ClustView/data/data_density_inc_5.npy')
    print(points.shape)

    points = points[1:2, :, :]
    print(points.shape)
    save_to_ply(points[0], './data/airplane.ply')

    # np.save('./data/airplane.npz', points)
    ...
    imgs = pc_views.get_img(points, resolution=256)
    ...
    for img in imgs:
        ...
        cv2.imwrite('./data/airplane_' + str(i) + '.jpg', img)
        cv2.imshow('img', img)
        cv2.waitKey(1000)
