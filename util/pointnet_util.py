...
def pc_normalize(pc):
    centroid = np.mean(pc, axis=0) #计算中心
    pc = pc - centroid
...
#     raw_size = idx.size()
#
#     idx = idx.reshape(raw_size[0], -1) # B S*K
#
#     res = torch.gather(points, 1, idx[..., None].expand(-1, -1, points.size(-1))) # 按指定的维度N进行索引
#
...
def farthest_point_sample(xyz, npoint):
    ...
    device = xyz.device
    B, N, C = xyz.shape
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device) # 返回的是索引
    distance = torch.ones(B, N).to(device) * 1e10 # 记录样本与该点的所有距离
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device) # 最远点坐标
    ...
    for i in range(npoint):
        ...
        dist = torch.sum((xyz - centroid) ** 2, -1) # 计算该点与所有点的距离，并存储在dist矩阵

        # distance = torch.min(distance, dist) # 更新distance，记录每个点与已出现采样点的最小距离
        mask = dist < distance
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    sqrdists = square_distance(new_xyz, xyz) #记录中心点与所有点的欧几里得距离
    group_idx[sqrdists > radius ** 2] = N    #大于radis^2的，设为N，其余的保留，前边距离没开根号
    group_idx = group_idx.sort(dim=-1)[0][:, :, :nsample] #带0为输出排序后的值，带1是输出索引，默认升序， B S nsample
    # 考虑到有可能前nsample个点中也有被赋值为N的点（即球形区域内不足nsample个点），这种点需要舍弃，直接用第一个点来代替即可
    mask = group_idx == N
    group_first = group_idx[:, :, 0].view(B, S, 1).repeat([1, 1, nsample])
    # 将这些点的值替换为距离中心最近的点
    group_idx[mask] = group_first[mask]   #返回的是索引
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, knn=False):
    ...
    Return:
        ...
        new_points: sampled points data, [B, S, k, 3+D]
        grouped_xyz: grouped points position data, [B, S, k, 3]
    """
    # 先用farthest_point_sample函数实现最远点采样FPS得到采样点的索引，再通过index_points将这些点的从原始点中挑出来，作为new_xyz
    B, N, C = xyz.shape
    S = npoint
    fps_idx = farthest_point_sample(xyz, npoint) # B N 3 -> # [B, S]
    new_xyz = index_points(xyz, fps_idx) # B S 3
    if knn:
        dists = square_distance(new_xyz, xyz)  # B x S x N
        idx = dists.argsort()[:, :, :nsample]  # B x S x k
    else:
        idx = query_ball_point(radius, nsample, xyz, new_xyz)
    grouped_xyz = index_points(xyz, idx) # [B, S, k, 3]
    grouped_xyz_norm = grouped_xyz - new_xyz.view(B, S, 1, C) # B S 1 3

    # 如果每个点上面有新的特征的维度，则用新的特征与旧的特征拼接，否则直接返回旧的特征
    if points is not None:
        grouped_points = index_points(points, idx) # [B, S, k, C]
        new_points = torch.cat([grouped_xyz_norm, grouped_points], dim=-1) # [B, S, k, C+D]
    else:
...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        new_points = torch.max(new_points, 2)[0].transpose(1, 2) # [B, S, D']
        return new_xyz, new_points
